...
function SearchContent() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["anime-search", dq],
    queryFn: () => fetchAnimeSearch(dq),
    enabled: dq.length >= 2,
  });

  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6">
      <h1 className="mb-6 text-2xl font-bold" style={{ fontFamily: "var(--font-display)" }}>Search Anime</h1>
      ...
      {dq.length < 2 ? <p className="text-sm text-white/30">Type at least 2 characters</p>
        ...
        : data?.data?.length ? (
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
            {data.data.map((item) => <ContentCard key={item.id} item={item} href={`/anime/${item.id}`} />)}
          </div>
