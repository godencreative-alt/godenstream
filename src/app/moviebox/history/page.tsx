    ...
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6">
      ...
      {h.length === 0 ? <p className="text-sm text-white/30">No history</p> : (
        <div className="space-y-2">{h.map((e, i) => (
          <Link key={`${e.content_id}-${i}`} href={`/moviebox/${e.content_id}/${e.episode_number}`} className="flex items-center gap-4 rounded-xl border border-white/[0.06] p-3 hover:bg-white/[0.03]">
