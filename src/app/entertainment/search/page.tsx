...
import { fetchEntertainmentSearch } from "@/lib/api";
...
function SearchContent() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["entertainment-search", debounced],
    queryFn: () => fetchEntertainmentSearch(debounced),
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
          {data.data.map((item) => (
            <ContentCard
              key={item.slug}
              item={item}
              href={`/entertainment/${encodeURIComponent(item.slug ?? "")}`}
            />
          ))}
        </div>
      ) : (
