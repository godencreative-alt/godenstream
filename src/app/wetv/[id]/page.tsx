...
import { fetchWetvDetail, fetchWetvEpisodes } from "@/lib/api";
import { Spinner } from "@/components/ui/Spinner";

export default function WetvDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { data: drama, isLoading } = useQuery({ queryKey: ["wetv-detail", id], queryFn: () => fetchWetvDetail(id) });
  const { data: episodes } = useQuery({ queryKey: ["wetv-episodes", id], queryFn: () => fetchWetvEpisodes(id) });
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      <div className="flex flex-col gap-6 md:flex-row md:gap-8">
        ...
        <div className="flex-1 space-y-4">
          ...
          {drama.chapter_count && <p className="text-[12px] text-white/40">{drama.chapter_count} Episodes</p>}
          {drama.introduction && <p className="text-sm leading-relaxed text-white/50">{drama.introduction}</p>}
          {episodes?.data?.[0] && (
            <Link href={`/wetv/${id}/${episodes.data[0].episode_index}`}
        ...
        <section className="mt-10">
          ...
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10">
            {episodes.data.map((ep) => (
              <Link key={ep.id} href={`/wetv/${id}/${ep.episode_index}`}
                className="flex items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.02] py-3 text-[13px] font-medium text-white/60 hover:border-[var(--dc-rose)]/30 hover:bg-[var(--dc-rose)]/5 hover:text-[var(--dc-rose)]">
                {ep.episode_index}
              </Link>
            ))}
          </div>
