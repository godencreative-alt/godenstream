...
export default function WetvBookmarksPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6">
      ...
      {bk.length === 0 ? <p className="text-sm text-white/30">Nothing saved</p> : (
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">{bk.map((i) => <ContentCard key={i.id} item={i} href={`/wetv/${i.id}`} />)}</div>
      )}
    </div>
  );
}
