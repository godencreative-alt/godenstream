...
function Content() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["wetv-search", dq], queryFn: () => fetchSearch(dq), enabled: dq.length >= 2 });

  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6">
      <h1 className="mb-6 text-2xl font-bold" style={{ fontFamily: "var(--font-display)" }}>Search WeTV</h1>
      ...
      {dq.length < 2 ? <p className="text-sm text-white/30">Type at least 2 characters</p>
        ...
        : data?.data?.length ? <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">{data.data.map((i) => <ContentCard key={i.id} item={i} href={`/wetv/${i.id}`} />)}</div>
        : <p className="text-sm text-white/30">No results</p>}
    </div>
  );
}

export default function WetvSearchPage() { return <Suspense fallback={<div className="flex justify-center py-20"><Spinner size="lg" /></div>}><Content /></Suspense>; }
